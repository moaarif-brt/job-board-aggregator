...
def fetch_company_jobs_workable(slug):
    ...
    Returns JSON dictionary with a "jobs" array.
    """
        ...
        if response.status_code == 200:
            ...
            if jobs:
                ...
                for job in jobs:
                    ...
                    city = loc_data.get("city", "") if isinstance(loc_data, dict) else ""
                    country = loc_data.get("country", "") if isinstance(loc_data, dict) else ""
                    location = ", ".join(filter(None, [city, country])) or "Not specified"
                    
                    remote, coords = enrich_location(location)
                    shortcode = job.get("shortcode", "")
                    job_url = f"https://apply.workable.com/{slug}/j/{shortcode}/" if shortcode else job.get("url")
...
def fetch_company_jobs_recruitee(slug):
    ...
    Returns JSON dictionary with an "offers" array, or a direct list of offers.
    """
            ...
            if jobs:
                ...
                for job in jobs:
                    ...
                    if not job_url and job.get("id"):
                        job_url = f"https://{slug}.recruitee.com/o/{job.get('id')}"
...
def fetch_company_jobs_personio(slug):
    """
    https://{slug}.jobs.personio.de/xml
    Returns XML feed of open positions.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "xml" not in content_type.lower():
                print(f"Skipping Personio for {slug}: returned non-XML content type ({content_type})")
                return slug, [], response.status_code
                
            try:
                root = ET.fromstring(response.content)
            except ET.ParseError:
                print(f"Error parsing XML for Personio slug {slug}")
                return slug, [], response.status_code
        ...
        print(f"Error fetching Personio for {slug}: {type(e).__name__}")
    except Exception as e:
        print(f"Error fetching Personio for {slug}: {e}")
    return slug, [], None
...
def fetch_company_jobs_jazzhr(slug):
    """
    https://app.jazz.co/feeds/export/jobs/{slug}
    Returns an XML feed of public JazzHR jobs.
    """
    url = f"https://app.jazz.co/feeds/export/jobs/{slug}"
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            company_name = root.findtext("company") or slug
            normalized = []
            for job in root.findall(".//job"):
                title = job.findtext("title")
                job_url = job.findtext("url")
                if not title or not job_url:
                    continue

                city = job.findtext("city") or ""
                state = job.findtext("state") or ""
                country = job.findtext("country") or ""
        ...
        print(f"Error fetching JazzHR for {slug}: {e}")
    return slug, [], None
